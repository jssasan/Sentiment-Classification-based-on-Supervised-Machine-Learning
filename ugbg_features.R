...
results_algo_runner_ugbg_features_only <- function(original_df, sparsity, category_name){
  ...
  ugbg_features_f300<- fetures_df_from_corpus(corpus = corpus_f300, ctrl_list = ctrl_list_ugbg, sparsity = sparsity_var)
  ugbg_feature_size <- dim(ugbg_features_f300)[2]
  ## running papa runner again on ngram features
  algo_runner_ugbg_f300<- cv_algo_runner(feature_df = ugbg_features_f300, rating_df = df_imp_firs300, feature_vector = c(1:ugbg_feature_size),seed = 500,nfold = 10)
  ...
  temp_colnames<- colnames(algo_runner_ugbg_f300)
  ...
  colnames(algo_runner_ugbg_f300) <- c("Performance_Metric", "Baseline_SCore")
  
  filename <-paste("UGBG_PRF_accuracy_mean_", category_name, sep = "")
  
  save(algo_runner_ugbg_f300, file =  paste(filename, ".Rda", sep = ""))
  
  write.csv(algo_runner_ugbg_f300, file = paste(filename, ".csv", sep = ""))
  
 
  return(algo_runner_ugbg_f300)
...
sink("PRFA_ugbg_2k_all_categories.txt")

UGBG_PRF_accuracy_mean_scores_2k_mob <- results_algo_runner_ugbg_features_only(original_df = df_imp_2k_f_mob_org,
                                       sparsity = 0.99,
                                       category_name = "mob_2k")



UGBG_PRF_accuracy_mean_scores_2k_cam <- results_algo_runner_ugbg_features_only(original_df = df_imp_2k_f_cam_org,
                                                                                      sparsity = 0.99,
                                                                                      category_name = "cam_2k")



UGBG_PRF_accuracy_mean_scores_2k_tab <- results_algo_runner_ugbg_features_only(original_df = df_imp_2k_f_tab_org,
                                                                                      sparsity = 0.99,
                                                                                      category_name = "tab_2k")



UGBG_PRF_accuracy_mean_scores_2k_lap <- results_algo_runner_ugbg_features_only(original_df = df_imp_2k_f_lap_org,
                                                                                      sparsity = 0.99,
                                                                                      category_name = "lap_2k")



UGBG_PRF_accuracy_mean_scores_2k_tv <- results_algo_runner_ugbg_features_only(original_df = df_imp_2k_f_tv_org,
                                                                                      sparsity = 0.99,
                                                                                      category_name = "tv_2k")



UGBG_PRF_accuracy_mean_scores_2k_vs <- results_algo_runner_ugbg_features_only(original_df = df_imp_2k_f_vs_org,
                                                                                      sparsity = 0.99,
                                                                                      category_name = "vs_2k")
